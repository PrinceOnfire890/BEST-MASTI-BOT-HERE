// Bott AI Companion - UID Specific Behavior + Code Generation + Google Mode
...
let BottEnabled = true; // Default to Bott being enabled
...
module.exports.config = {
    ...
    description: "Bott, your AI companion: modern, smart, flirty with users, roasts playfully, and super respectful to Owner. UID specific behavior. Responds only when triggered. Modified for 3-4 line replies (with code exceptions). Speaks in user's preferred language (Hinglish/Punjabi/etc.). Owner can turn Bott on/off. Now with Google-like information search!",
    commandCategory: "AI-Companion",
    usages: "Bott [आपका मैसेज] / Bott code [आपका कोड प्रॉम्प्ट] (Owner Only) / Bott on (Owner Only) / Bott off (Owner Only) / Bott google on (Owner Only) / Bott google off (Owner Only) / Reply to Bott",
    cooldowns: 2,
};
...
// Function to toggle Bott's on/off state for the owner
async function toggleBottOnOff(body, senderID, api, threadID, messageID) {
    if (senderID !== ownerUID) {
        return null;
    }

    if (body.toLowerCase().includes("Bott on")) {
        if (BottEnabled) {
            return "Mai pehle se hi ON hu Boss! 😉";
        }
        BottEnabled = true;
        return "Mai aa gayi Boss! Bolo kya karna hai? 😎";
    } else if (body.toLowerCase().includes("Bott off")) {
        if (!BottEnabled) {
            return "Mai pehle se hi OFF hu Boss. 😴";
        }
        BottEnabled = false;
        return "Okay Boss, mai OFF ho gayi. Jab bulana, 'Bott on' bol dena. 👋";
    }
    return null;
}
...
async function toggleGoogleMode(body, senderID, api, threadID, messageID) {
    ...
    if (body.toLowerCase().includes("Bott google on") || body.toLowerCase().includes("google on")) {
        if (googleMode) {
    ...
    } else if (body.toLowerCase().includes("Bott google off") || body.toLowerCase().includes("google off")) {
        if (!googleMode) {
            return "Boss, Google mode pehle se hi OFF hai. 😴";
        }
        googleMode = false;
        return "Okay Boss, Google mode OFF. Ab main wapas apni purani Bott ban gayi. 😉";
    }
    return null;
}
...
module.exports.handleEvent = async function ({ api, event }) {
    try {
        ...
        const isBottTrigger = body?.toLowerCase().startsWith("Bott");
        const isReplyToBott = messageReply?.senderID === api.getCurrentUserID();

        // Check for Bott on/off command first, it should always work for owner
        const onOffResponse = await toggleBottOnOff(body, senderID, api, threadID, messageID);
        if (onOffResponse) {
        ...
        // If Bott is disabled and it's not an on/off command, just return
        if (!BottEnabled && senderID !== ownerUID) {
            return;
        }
        if (!BottEnabled && senderID === ownerUID && !body.toLowerCase().includes("Bott on")) {
            return;
        }

        if (!(isBottTrigger || isReplyToBott)) {
            return; // Ignore messages that are not triggers
        }

        console.log("--- Bott HandleEvent ---");
        console.log("Bott's Bot ID:", api.getCurrentUserID());
        console.log("Sender ID:", senderID);
        console.log("Is Owner UID:", senderID === ownerUID);
        console.log("Message Body:", body);
        console.log("Bott Enabled:", BottEnabled);
        ...
        if (userMessageRaw.toLowerCase().startsWith("code ")) {
            ...
            if (!userMessageForAI) {
                api.sendTypingIndicator(threadID, false);
                return api.sendMessage("क्या कोड चाहिए? 'Bott code [आपका प्रॉम्प्ट]' ऐसे लिखो।", threadID, messageID);
            }
        } else {
        ...
        const basePromptInstructions = `Apne jawab hamesha casual, smart, charming aur **sirf 1-2 lines mein hi dena (lekin agar code generate karna pade ya Google search result ho, toh answer lamba ho sakta hai)**. ${promptLanguageInstruction} Tum hamesha user ki bhasha mein hi baat karna.`;

        if (isExplicitCodeRequest) {
            promptToSendToProxy = `CODE_GEN_REQUEST: ${userMessageForAI}`;
            console.log("Bott Bot: Sending explicit code generation request to proxy.");
        ...
        try {
            ...
            if (!botReply || botReply.toLowerCase().startsWith("user:") || botReply.toLowerCase().startsWith("Bott:")) {
                if (senderID === ownerUID) {
                ...
                if (!isExplicitCodeRequest && !googleMode) { // Only add to history for general chat
                    chatHistories[senderID].push(`Bott: ${botReply}`);
                }
            }
            ...
            if (isExplicitCodeRequest || googleMode) { // No emoji or footer for code generation or Google mode
                ...
                if (isBoldMode || hornyMode) {
                    replyText = `${botReply} 😉🔥💋\n\n_Your charmingly naughty Bott... 😉_`;
                } else {
                    replyText = `${botReply} 😊💖✨`;
                }
            } else {
        ...
        console.error("Bott Bot Catch-all Error:", err);
